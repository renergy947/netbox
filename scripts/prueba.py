...
class AutomatizarDespachoScript(Script):
    class Meta:
        ...
        description = "Formulario unificado para la creación automatizada de despachos y activos."
    ...
    num_despacho = StringVar(label="Nombre/Número del Despacho")
    tomas_red = IntegerVar(default=2, label="Tomas RJ45 de pared")
    
    tipo_pc = ChoiceVar(choices=(('torre', 'Torre'), ('portatil', 'Portátil')), default='torre', label="Tipo de PC")
    ...
    num_teclados = IntegerVar(default=1, label="Teclados USB")
    num_ratones = IntegerVar(default=1, label="Ratones USB")
    ...
    def run(self, data, commit):
        ...
        despacho.save()
        self.log_success(f"Ubicación creada: {despacho_name}")
        ...
        if data['poner_telefono']:
            ...
            tel.save()
            self.log_success(f"Teléfono creado: {tel.name}")
